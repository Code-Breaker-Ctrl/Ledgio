...
(function() {
        ...
        <article class="goal-card" data-id="${escapeHtml(goal.id)}">
          <div class="goal-card-top">
            ...
            <div style="display: flex; gap: 4px;">
              <button type="button" class="goal-card-menu-btn" data-action="history" data-id="${escapeHtml(goal.id)}" title="View History" aria-label="View History">
    ...
    if (historyList) {
      if (deposits.length === 0) {
        ...
        historyList.innerHTML = deposits.map(d => {
          ...
          const isNeg = amt < 0;
          const amtStr = (isNeg ? '-' : '+') + formatCurrency(Math.abs(amt), true);
  ...
  function setupGoalsEventListeners() {
    ...
    document.querySelector('.goals-filter-group')?.addEventListener('click', (e) => {
  ...
  // =========================================================================
  // Phase 5: Loans & Debts Module Implementation
  // =========================================================================
  ...
  function renderLoans() {
    ...
    if (toReceiveEl) {
      ...
      if (isStealthModeActive) toReceiveEl.classList.add('stealth-masked');
      else toReceiveEl.classList.remove('stealth-masked');
    }

    if (toPayEl) {
      toPayEl.textContent = formatCurrency(totalToPay);
      if (isStealthModeActive) toPayEl.classList.add('stealth-masked');
    ...
    personMap.forEach((group) => {
      ...
      const cardsHtml = filteredLoans.map(loan => {
        ...
        const dirText = isLent ? (details.isSettled ? 'Lent (Settled)' : 'They owe you') : (details.isSettled ? 'Borrowed (Settled)' : 'You owe them');
        ...
        return `
          <article class="loan-card" data-id="${escapeHtml(loan.id)}">
            ...
            <p class="loan-notes-text" title="${escapeHtml(loan.notes || '')}">${escapeHtml(loan.notes || 'No description provided')}</p>

            <div class="loan-amount-row">
              <div class="loan-outstanding-val ${isStealthModeActive ? 'stealth-masked' : ''}">${outstandingStr}</div>
              <div class="loan-principal-val">Principal: <span class="${isStealthModeActive ? 'stealth-masked' : ''}">${principalStr}</span></div>
            </div>
  ...
  function saveLoan() {
    ...
    if (!loanDate) {
      showToast('Please select a loan date', 'warning');
      return;
    }
  ...
  function openSettlementModal(loanId) {
    ...
    const details = getLoanDetails(loan);
    if (details.isSettled) {
      showToast('This loan is already fully settled!', 'info');
      return;
    }
  ...
  function saveSettlement() {
    ...
    if (!date) {
      showToast('Please select a settlement date', 'warning');
      return;
    }
  ...
  function saveRenamePerson() {
    ...
    if (modifiedCount > 0) {
      saveData();
      renderLoans();
      showToast(`Renamed "${oldName}" to "${newName}" across ${modifiedCount} record${modifiedCount === 1 ? '' : 's'}`, 'success');
    }
  ...
  function setupEventListeners() {
    ...
    document.getElementById('reset-data')?.addEventListener('click', async () => {
      ...
      if (confirmed) {
        ...
        state = { version: 2, income: 0, expenses: [], budgets: {}, goals: [], goal_deposits: [], loans: [], loan_settlements: [], settings: { currency: 'INR', darkMode: false } };
        populateDropdowns();
        refreshUI();
        applyDarkMode();
        updateSyncStatusUI();
