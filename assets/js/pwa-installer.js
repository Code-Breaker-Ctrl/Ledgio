...
(function() {
  ...
  // Capture BeforeInstallPromptEvent
  ...
  // 5. Fallback Instructions Modal Builder for Unsupported Browsers & Devices
  ...
  // 6. Trigger Install Flow (Rock-Solid Multi-Tap Fix & Immediate Feedback)
  async function triggerInstallFlow(clickedBtn) {
    ...
    // Case 1: Native BeforeInstallPromptEvent is ready
    if (deferredPrompt) {
    ...
    // Case 2: Early tap before beforeinstallprompt has arrived (within initial 3.5s)
    ...
    if (elapsedSinceLoad < 3500 && !hasBeforeInstallPromptFired && !isAwaitingPrompt) {
      ...
      // Give visual feedback on the clicked button
      ...
      // Wait up to 1200ms for beforeinstallprompt to arrive
    ...
    // Case 3: Unsupported browser, iOS, in-app webview, or dismissed native prompt
    showInstallGuideModal();
  }
  ...
  // 7. App Installed Event & Analytics Logging
  ...
  // 8. Online / Offline Connectivity Monitor
  ...
  window.addEventListener('online', () => {
    updateNetworkStatus();
    if (window.showToast && !window.__ledgio_syncEngineActive) {
  ...
  // 9. Privacy-First App Telemetry & Install Analytics
  ...
  function trackSessionLaunch() {
    if (!sessionStorage.getItem('ledgio_session_logged')) {
      ...
      const log = () => logAppAnalytics('app_launch');
      if ('requestIdleCallback' in window) {
        requestIdleCallback(log, { timeout: 3000 });
      } else {
        setTimeout(log, 2000);
      }
    }
